...
public class Evaluator {
	...
	public static double calculateMAE(DataSet data, Model model) throws Exception{
		double error = 0;
		int dbsize = (Integer)(data.getDataSetInfo().get(Constants.TOTALRATINGS));
		for(UserRatings urs = data.nextUser(); urs != null;  urs = data.nextUser()){
			for(RatingInfo ri = urs.getNormalNextRating(); ri!= null; ri = urs.getNormalNextRating() ){
				...
				if (predict == -1){ //error in prediction 
					...
					error += Math.abs(ri.rating - predict);
				}
			}
	...
	public static double calculateMAE2(DataSet data, Recommender recommender) throws Exception{
		double error = 0;
		int dbsize = (Integer)(data.getDataSetInfo().get(Constants.TOTALRATINGS));
		System.out.println(dbsize);
		for(UserRatings urs = data.nextUser(); urs != null;  urs = data.nextUser()){
			...
			if (predicts == null){
				...
				for(RatingInfo ri = urs.getNormalNextRating(); ri!= null; ri = urs.getNormalNextRating() ){
					error += Math.abs((ri.rating - predicts[ri.itemId]));
				}
			}
			if (urs.uid % 2000 == 0){
				System.out.print(".");
